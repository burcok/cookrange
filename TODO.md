...
| `CHAT-03` | 🚧 | Scope group chat images to participants | High | M | 2–3 d | v0.9.8 / M2 | Security | see `SEC-13`; `storage.rules` (group chat images readable by any authenticated user) | `SEC-13` | **Chat Upgrade Faz 5 (2026-08-07)**: a real fix now exists but is additive, not a migration — new group-chat image uploads go through `StorageUploadService.uploadGroupChatMedia`'s `chat_media/{chatId}/{uid}/{fileName}` path (`storage.rules`: `allow read: if false` always; resolved only via the `getChatMediaUrl` callable, which re-verifies real chat participant/group-membership server-side before minting a short-lived signed URL). The OLD `chat_images/{scopeId}/` path and every image already uploaded there remain exactly as unscoped as before — no existing call site was switched over, and the rule comment's "security by obscurity" caveat still applies to all of that legacy content. Close this card only once every group-chat image call site adopts the new path | Chat images are the most private content in the app |
| `CHAT-04` | ✅ | Chat pagination for long histories | Medium | S | 1–2 d | v1.0.0 / M4 | Flutter Engineer | `chat_detail_screen.dart`, `chat_service.dart:getMessagesPage` | `FB-17` | **Stale row, corrected 2026-08-07**: this was marked ❌ but has been built since Faz 2 §2.2 (well before this correction) — `getMessagesPage` is cursor-paginated (`startAfterDocument`, bounded `.limit()`), wired to `chat_detail_screen.dart`'s reverse-infinite-scroll (`_onScroll`/`_loadOlder`). Verified working, not re-tested against an actual 5,000-message thread | — |
| `CHAT-05` | ✅ | Chat message delete / unsend | Medium | S | 2 d | v1.0.0 / M4 | Flutter Engineer | `chat_service.dart:deleteMessageForEveryone/deleteMessageForMe`, `storage_upload_service.dart:deleteByUrl/deleteByPath` | — | **Stale row, corrected 2026-08-07**: delete-for-me and delete-for-everyone both exist (Faz 2 §2.1), rules-enforced 15-minute window, plus a separate moderator-delete path (Faz 2 §2.6). **Fixed same day**: `deleteMessageForEveryone` cleared the Firestore `attachments` array but never removed the underlying Storage object — found while correcting this row, not left as documented debt. Now takes the caller's already-loaded `attachments` (no extra read) and best-effort deletes each one (new `StorageUploadService.deleteByPath` alongside the existing `deleteByUrl`, covering both the legacy `chat_images/` URL shape and the new `chat_media/` bare-path shape) after the Firestore update succeeds, fire-and-forget so a Storage failure never blocks the message delete itself | Deleting one side of a conversation has product and legal nuance — document the choice. The orphaned-Storage-object gap was also a `BLK-12` (GDPR erasure) completeness question, not just cost — now closed for future deletes; **not** backfilled for messages already deleted before this fix |
| `CHAT-06` | ❌ | Block enforcement in chat | High | S | 1 d | v0.9.8 / M2 | Security | `chat_service.dart`, `users/{uid}/block_list` | `COM-06` | A blocked user cannot open or send to the blocker's chat, enforced in rules. **Confirmed still accurate 2026-08-07** — block enforcement is client-side only everywhere in this app (`S13`), chat included | Blocking that does not stop DMs is not blocking |
| `CHAT-07` | 🚧 | Chat moderation and reporting flow into the queue | Medium | S | 2 d | v1.0.0 / M4 | Moderation | `chat_service.dart:reportMessage/reportUser`, `admin_reports_screen.dart` | `BLK-05`, `MOD-01` | **Stale row, corrected 2026-08-07**: the underlying gap this card names (`reportMessage`/`reportUser` wrote only `created_at`/`targetAuthorUid`, while the admin queue reads `timestamp`/`authorId` — so a chat report was silently invisible to admins despite writing successfully) was found and fixed in Faz 2 §2.6; both methods now write the canonical shape and route into the same queue as every other report. **Not verified**: whether the admin-side reviewing UI limits thread exposure to the minimum necessary (this card's other acceptance criterion) — no admin session has exercised the report queue against a real chat report yet | Admin access to private messages is a serious privacy decision — define the minimum viable disclosure |
