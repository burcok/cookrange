...
| Food logging — today's plan / quick add / recipe / AI-scanned / barcode | ✅ | v0.4 | `FoodLogService` | `quick_add_sheet.dart` is now wired into the quick-actions menu (Faz 8 audit follow-up, previously had zero navigational callers) — a real 5th logging method again |
| Recent & frequent foods | ✅ | v0.6 | `RecentFoodService` (Hive) | Reachable via the quick-actions menu's Quick Add entry |
...
| Friend requests — auto-accept toggle (Settings > Privacy > "Friend Requests") | 🚧 | v0.9.6 | `UserModel.autoAcceptFriendRequests`, `functions/social.js` (`sendFriendRequest`) | Found and closed same-session: the row rendered a hardcoded "Auto Accept" subtitle + chevron with **no `onTap`, no backing field anywhere (`UserModel`, Firestore, `functions/`), and no server logic** — purely a dead affordance, unlike every sibling row in that section. Now backed end to end: an owner-writable `auto_accept_friend_requests` field (same pattern as `is_private`, not in `firestore.rules`' `touchesProtectedUserFields()` denylist) drives a real `Switch.adaptive`; `sendFriendRequest` checks the *recipient's* flag and, when on, skips the pending `friend_requests` pair entirely and creates the friendship directly, notifying the sender via the existing `friendAccepted` path (deliberately not the `friendRequest` type, which would render Accept/Reject buttons over a request doc that in this branch never exists). Code-level only — not deployed or exercised against live Firestore/Cloud Functions in this environment |
